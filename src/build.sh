#!/bin/bash
# ============================================================
# NoMount nm Binary Build Script
# ============================================================
# Mimics the original nomount build process using zig compiler
...
SRC_FILE="$SCRIPT_DIR/nm.c"
OUT_DIR="$SCRIPT_DIR/../module"

# Zig compiler flags for minimal binary size (from original nomount)
ZIG_FLAGS=(
...
build_arm64() {
    ...
    zig cc -target aarch64-linux "${ZIG_FLAGS[@]}" "$SRC_FILE" -o "$OUT_DIR/nm-arm64"

    echo "  Before sstrip: $(wc -c < "$OUT_DIR/nm-arm64") bytes"

    if [ -f "$SCRIPT_DIR/sstrip" ]; then
        "$SCRIPT_DIR/sstrip" -z "$OUT_DIR/nm-arm64" 2>/dev/null || true
        echo "  After sstrip:  $(wc -c < "$OUT_DIR/nm-arm64") bytes"
    fi

    file "$OUT_DIR/nm-arm64"
}
...
build_arm() {
    ...
    zig cc -target arm-linux "${ZIG_FLAGS[@]}" "$SRC_FILE" -o "$OUT_DIR/nm-arm"

    echo "  Before sstrip: $(wc -c < "$OUT_DIR/nm-arm") bytes"

    if [ -f "$SCRIPT_DIR/sstrip" ]; then
        "$SCRIPT_DIR/sstrip" -z "$OUT_DIR/nm-arm" 2>/dev/null || true
        echo "  After sstrip:  $(wc -c < "$OUT_DIR/nm-arm") bytes"
    fi
...
clean() {
    ...
    rm -f "$OUT_DIR/nm-arm64" "$OUT_DIR/nm-arm"
...
main() {
    echo "============================================================"
    echo " NoMount nm Binary Builder"
    echo "============================================================"
    ...
    ls -la "$OUT_DIR"/nm-* 2>/dev/null || true
}
