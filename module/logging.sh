#!/system/bin/sh
# ============================================================
# NoMount Unified Logging Library
# ============================================================
# Provides structured, organized logging for all NoMount components.
#
# Log Directory Structure:
#   /data/adb/nomount/logs/
...
# CONFIGURATION
# ============================================================
NOMOUNT_BASE="${NOMOUNT_BASE:-/data/adb/nomount}"
NOMOUNT_LOG_BASE="$NOMOUNT_BASE/logs"

# Log directories
LOG_DIR_KERNEL="$NOMOUNT_LOG_BASE/kernel"
LOG_DIR_FRONTEND="$NOMOUNT_LOG_BASE/frontend"
LOG_DIR_SUSFS="$NOMOUNT_LOG_BASE/susfs"
LOG_DIR_ARCHIVE="$NOMOUNT_LOG_BASE/archive"
...
log_create_dirs() {
    ...
    chmod 755 "$NOMOUNT_LOG_BASE" 2>/dev/null
}
...
log_init() {
    ...
    case "$component" in
        service|post-fs-data|monitor|sync|uninstall)
...
# Collect NoMount kernel logs from dmesg
...
log_collect_kernel() {
...

    {
        ...
        echo "========================================"
        dmesg 2>/dev/null | grep -iE "nomount|vfs_dcache|susfs" | tail -200
...
log_early() {
    local msg="$1"
    local early_log="$NOMOUNT_BASE/early_boot.log"
    mkdir -p "$NOMOUNT_BASE" 2>/dev/null
...
log_get_files() {
    ...
    case "$component" in
        ...
        susfs)
            ...
            ;;
        all)
            ls -la "$NOMOUNT_LOG_BASE/"*/*.log 2>/dev/null
            ;;
    esac
}
